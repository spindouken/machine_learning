...
class DeepNeuralNetwork:
    ...
    def train(self, X, Y, iterations=5000, alpha=0.05,
        ...
        # Train the network of the specified number of iterations
        for i in range(iterations + 1):
            ...
            # Save the cost every `step` iterations of graphing
    ...
    def save(self, filename):
        ...
        """
        # Check if filename has the extension .pkl, if not add it
        if not filename.endswith('.pkl'):
            filename += '.pkl'

        # Open the file in write mode (binary)
        with open(filename, 'wb') as file:
            # Use pickle to dump the object to file
