...
from tensorflow.keras.layers import BatchNormalization, ReLU, LeakyReLU, Input, Embedding, Concatenate
...
def load_data(data_dir, batch_size=32):
    ...
    counter = Counter(trainingGenerator.classes)
    max_val = float(max(counter.values()))
    class_weights = {class_id: max_val / num_images for class_id, num_images in counter.items()}

    return trainingGenerator, validationGenerator, class_weights


def build_generator(noise_dim=100, num_classes=4):
    noise = Input(shape=(noise_dim,))
    label = Input(shape=(1,), dtype='int32')
    

    half_noise_dim = noise_dim // 2
    label_embedding = Embedding(num_classes, half_noise_dim)(label)
    label_embedding = Flatten()(label_embedding)
    print("Shape of label_embedding after Flatten:", label_embedding.shape)

    model_input = Concatenate(axis=1)([noise, label_embedding])

    model = Sequential([
        Dense(8 * 8 * 128, input_dim=noise_dim + half_noise_dim),
        Reshape((8, 8, 128)),
    ...
    final_generator = Model([noise, label], model(model_input))

    return final_generator

def build_discriminator(input_shape=(128, 128, 3), num_classes=4):
    img = Input(shape=input_shape)
    label = Input(shape=(1,), dtype='int32')
    
    label_embedding_size = 50  # Adjust this size
    label_embedding = Embedding(num_classes, label_embedding_size)(label)
    label_embedding = Flatten()(label_embedding)
    print("Shape of label_embedding after Flatten:", label_embedding.shape)
    ...
    model = Sequential([
        Conv2D(64, kernel_size=4, strides=2, padding="same", input_shape=(128, 128, 4)),
        LeakyReLU(alpha=0.2),
    ...
    final_discriminator = Model([img, label], model(model_input))
...
def gan_training(generator, discriminator, trainingGenerator, use_wandb=True, noise_dim=100, epochs=100, batch_size=32, patience=3):
    ...
    for epoch in range(epochs):
        ...
        real_imgs, labels = trainingGenerator.next()
        ...
        d_loss_real = discriminator.train_on_batch([real_imgs, labels], real_y, class_weight=class_weights)
        d_loss_fake = discriminator.train_on_batch([fake_imgs, labels], fake_y)
...
if __name__ == "__main__":
    ...
    trainingGenerator, validationGenerator, class_weights = load_data('alzClassification/data/')
